// Return the number of total permutations of the provided string that 
// don't have repeated consecutive letters. Assume that all characters 
// in the provided string are each unique.

// For example, aab should return 2 because it has 6 total permutations 
// (aab, aab, aba, aba, baa, baa), but only 2 of them (aba and aba) don't
// have the same letter (in this case a) repeating.



const allPerms = (str) => {
  if(str.length === 1) return str
  const perms = []
  for(let i = 0; i< str.length; i++){
    const letter = str[i]
    const restOfString = str.substr(0, i) + str.substr(i+1)
    const restOfPerms = allPerms(restOfString)
    for(let perm of restOfPerms){
      perms.push(letter + perm)
    }
  }
  return perms
}

function permAlone(str) {
  if(str.length === 1) return 1
  const perms = allPerms(str)
    .filter(each => 
      each.split("").reduce((prev, curr) => {
        if(prev[0] !== curr) return [curr, prev[1]]
        else return ["", false]
      }, ["", true])[1]
    )
  return perms.length;
}

permAlone("a")

permAlone('aab');